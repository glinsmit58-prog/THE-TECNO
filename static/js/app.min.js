...
// V61: Smooth fade-in for game cover images on every page that uses
// .v60-game-cover (home, all_games, products groups). Removes the white
// flash that lazy-loaded artwork shows on Android browsers — the bug that
// looked like screen tearing/static when scrolling into the games section.
(function () {
  ...
  function init() {
    var imgs = document.querySelectorAll(
      '.v60-game-cover img, .v60-hero-card-frame img'
    );
...
// Polls /api/wallet on focus/visibility-change so the navbar number updates
// shortly after a deposit is approved or an order is placed, without the user
...
(function () {
  ...
  function refresh() {
    ...
    fetch('/api/wallet', { credentials: 'same-origin', headers: { 'Accept': 'application/json' } })
      ...
      .then(function (j) {
        ...
        // The previous logic kept the *old* suffix while replacing only the
        // numeric portion, which produced "8.20 ل.س" for a USD value, or
        // a SYP value with "$" — exactly the bug users hit after a deposit.
  ...
  // Public hook: pages can call window.tgRefreshBalance() right after an
  // AJAX action (e.g. a successful POST to /api/orders) to update the
  // navbar without waiting for the next visibility/focus event.
  ...
  // Light polling every 25s while the tab is in the foreground.
